...
// Lightweight DTO for unread counts fetched directly from the homeserver.
class UnreadCountsData {
  const UnreadCountsData({required this.notification, required this.highlight});

  factory UnreadCountsData.fromJson(Map<String, dynamic> json) {
    return UnreadCountsData(
      notification: (json['notification_count'] as num?)?.toInt() ?? 0,
      highlight: (json['highlight_count'] as num?)?.toInt() ?? 0,
    );
  }
...
/// Fetch server-provided unread notification counts for a room using a
/// lightweight classic sync: `GET /_matrix/client/v3/sync?timeout=0`.
///
/// Returns the `unread_notifications` counts from the joined room entry.
/// This aligns with Synapse behavior even when Sliding Sync omits counts.
Future<RustResult<UnreadCountsData>> rustRoomFetchServerUnreadCounts({
  required String homeserverUrl,
  ...
  try {
    ...
    // Use a short long-poll (10s) to give the server a moment to
    // incorporate the new event into unread counters while keeping
    // latency bounded for tests.
    final uri = base.replace(
      path: '/_matrix/client/v3/sync',
      queryParameters: {'timeout': '10000'}, // milliseconds
    );
    ...
    try {
      ...
      if (resp.statusCode != 200) {
        return RustResult(
          ok: false,
          data: null,
          error: 'HTTP ${resp.statusCode}: $body',
        );
      }
      final obj = jsonDecode(body) as Map<String, dynamic>;
      final rooms = (obj['rooms'] as Map?) ?? const {};
      final joined = (rooms['join'] as Map?) ?? const {};
      final room = (joined[roomId] as Map?) ?? const {};
      ...
      return RustResult(ok: true, data: UnreadCountsData(notification: n, highlight: h), error: null);
    } finally {
      client.close(force: true);
    }
  } catch (e) {
    return RustResult(ok: false, data: null, error: e.toString());
  }
}
