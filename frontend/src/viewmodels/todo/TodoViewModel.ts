...
import type { NewTodoItem, UpdateTodoItem, TodoList, TodoItem, User } from '../../api/generated/models';
...
export class TodoViewModel implements IModuleViewModel {
  ...
  private todoApi: DefaultApi;
    ...
    public async fetchAndTransformTodos(): Promise<void> {
        try {
            const todoLists: TodoList[] = await this.todoApi.getTodoListsByUserId({ userId: cloudAuthViewModel.userID || '' });
            ...
            for (const list of todoLists) {
                ...
                allTimelineItems.push({
                    ...
                    timestamp: list.updatedAt ? list.updatedAt.getTime() : list.createdAt ? list.createdAt.getTime() : 0,
                    listId: list.id,
                });
                ...
                const transformedItems: TimelineItem[] = todoItems.map(item => ({
                    ...
                    timestamp: item.updatedAt ? item.updatedAt.getTime() : item.createdAt ? item.createdAt.getTime() : 0,
    ...
    async updateTodoItem(listId: string, itemId: string, updateTodoItem: UpdateTodoItem): Promise<void> {
        try {
    ...
    async createTodoItem(listId: string, title: string, description: string, dueDate: Date | undefined): Promise<void> {
        try {
            ...
            if (prevPosition) {
                prevPosition = (todoItems.find(item => item.id === prevPosition) as any)?.position || null;
            }
            if (nextPosition) {
                nextPosition = (todoItems.find(item => item.id === nextPosition) as any)?.position || null;
            }
    ...
    async updateTodoItemPosition(itemId: string, listId: string, prevItemId: string | null, nextItemId: string | null): Promise<void> {
        try {
            const todoItems = await this.todoApi.getTodoItemsByListId({ listId });
            let prevPosition: string | null = null;
            let nextPosition: string | null = null;
