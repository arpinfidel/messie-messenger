...
func (e *BridgeHTTPError) Error() string {
	return fmt.Sprintf("bridge http error %d: %s", e.StatusCode, string(e.Body))
}
...
func (a *Adapter) doGet(ctx context.Context, path string, q url.Values) (*http.Response, error) {
	...
	if resp.StatusCode/100 != 2 {
		...
		resp.Body.Close()
		return nil, &BridgeHTTPError{StatusCode: resp.StatusCode, Body: b}
	}
	return resp, nil
}
...
func (a *Adapter) StartConnection(ctx context.Context, userID uuid.UUID, mxid string) (*StartResult, error) {
	...
	connID := ""
	if v, ok := raw["process_id"].(string); ok {
		connID = v
	}
	if connID == "" {
		if v, ok := raw["login_id"].(string); ok {
			connID = v
		}
	}
	// Extract QR payload when provided (data or image_url). Prefer raw data string.
	var qr *string
	if v, ok := raw["data"].(string); ok && v != "" {
		qr = &v
	}
	// Expiry (best-effort)
...
func (a *Adapter) Logout(ctx context.Context, mxid string) error {
	...
	if resp.StatusCode/100 != 2 {
		return fmt.Errorf("bridge logout %d", resp.StatusCode)
	}
	return nil
}
...
func (a *Adapter) ListLogins(ctx context.Context, mxid string) ([]string, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("logins %d", resp.StatusCode)
	}
	var out struct {
...
func (a *Adapter) getLoginFlows(ctx context.Context, mxid string) (map[string]any, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("flows %d", resp.StatusCode)
	}
...
func (a *Adapter) startLoginRaw(ctx context.Context, mxid, flow string) (map[string]any, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("start %d", resp.StatusCode)
	}
	var out map[string]any
	if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
		return nil, err
...
func mapToLoginStep(out map[string]any) (LoginStep, error) {
	...
	if v, ok := out["process_id"].(string); ok {
		proc = &v
	}
	if v, ok := out["login_id"].(string); ok {
		login = &v
	}
	if v, ok := out["step_id"].(string); ok {
		step = &v
	}
	switch t {
	...
	case "complete":
		var id *string
		if comp, ok := out["complete"].(map[string]any); ok {
			if v, ok := comp["user_login_id"].(string); ok {
				id = &v
			}
		}
...
func (a *Adapter) submitLoginStepRaw(ctx context.Context, mxid, processID, stepID, action string, body map[string]any) (map[string]any, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("step %d", resp.StatusCode)
	}
	var out map[string]any
	if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
		return nil, err
...
func (a *Adapter) WhoamiRaw(ctx context.Context, mxid string) (map[string]any, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("whoami %d", resp.StatusCode)
	}
	var out map[string]any
	if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
		return nil, err
...
func (a *Adapter) Whoami(ctx context.Context, mxid string) (*generated.BridgeWhoamiResponse, error) {
			...
			if m, ok := it.(map[string]any); ok {
				...
				if p, ok := m["profile"].(map[string]any); ok {
					...
					profile = &struct {
						DisplayName *string `json:"displayName,omitempty"`
						ExternalId  *string `json:"externalId,omitempty"`
					}{DisplayName: dn, ExternalId: eid}
				}
				var state *string
				if s, ok := m["state"].(map[string]any); ok {
					if lbl, ok := s["state_event"].(string); ok {
						state = &lbl
					}
				}
	...
	if v, ok := out["homeserver"].(string); ok {
		resp.Homeserver = &v
	}
	if v, ok := out["bridge_bot"].(string); ok {
		resp.BridgeBot = &v
	}
	if v, ok := out["command_prefix"].(string); ok {
		resp.CommandPrefix = &v
	}
	return resp, nil
}
